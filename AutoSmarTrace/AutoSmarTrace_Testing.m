function AutoSmarTrace(input_path,out_filename)
...
X = chains(cn).points(1,1:pt_sep:end);
Y = chains(cn).points(2,1:pt_sep:end);
...
[sp_val, dir_der, xxx, kappa] = mod_interparc2(1e-6:L2,DOTS(1,1:3:end)*nmperpx,DOTS(2,1:3:end)*nmperpx,'spline');
...
figure(1)
hold on
plt = plot(X, Y, 'ko','MarkerSize',10,'linewidth',1);

function [pt,dudt,fofthandle, curvature] = mod_interparc2(l,px,py,varargin)
...
%   tbins((tbins >= n) | (t >= 1)) = NaN;
  ...
  if nargout > 1
    ...
    %NRtest: parametric second derivative:
    du2dt = (dudt(tbins+1,:) - dudt(tbins,:))./repmat(chordlen(tbins),1,ndim);
  end
...
for i = 1:ndim
  ...
  %xp.coefs = xp.coefs*diffarray;
  xp.order = 2;
  xp.coefs = bsxfun(@times, xp.coefs(:,1:end-1), xp.order:-1:1);
  spld2{i} = xp;
end
...
if nargout > 1
  ...
  for L = 1:ndim
    dudt2(:,L) = ppval(spld2{L},ti);
  end

  % NRtest remove NaNs at the end
  if firstnan
     dudt = dudt(1:firstnan-1,:);
     dudt2 = dudt2(1:firstnan-1,:);
  end
  
%   curvature = dudt2./(1 + dudt.^2).^(3/2);
curvature = (dudt(:,1).*dudt2(:,2)-dudt(:,2).*dudt2(:,1))./...
    (sqrt(dudt(:,1).^2 + dudt(:,2).^2)).^3;

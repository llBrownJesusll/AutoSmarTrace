function AutoSmarTraceTest(input_path,out_filename,nmperpix,varargin)
...
% --- VISUALIZATION 1: Initial sparse points and B-spline fit ---
figure(2);
imshow(handles.A);
hold on;
plot(Y, X, 'yo', 'MarkerSize', 5, 'LineWidth', 2); % Original points in yellow
plot(Ysp, Xsp, 'c-', 'LineWidth', 2); % Initial spline in cyan
title('Initial Sparse Points and B-Spline Fit');
legend('Initial Points', 'Initial B-Spline');
hold off;
pause(1); % Pause to view the figure
% ----------------------------------------------------------------
...
% --- VISUALIZATION 2: Filtered Image ---
figure(3);
imshow(im1);
title('Filtered Image (Top-hat and Median)');
pause(1); % Pause to view the figure
% -----------------------------------------
...
% --- VISUALIZATION 3: Perpendicular Search ---
figure(4);
imshow(im1);
hold on;
plot(Ysp, Xsp, 'c-', 'LineWidth', 2); % Initial spline
title('Perpendicular Search for Fibril Center');
% --------------------------------------------
...
for LL=1:length(Xsp)
  % tangent direction
    ...
    % --- VISUALIZATION 3 (continued): Plot search lines ---
    if mod(LL, 10) == 0 % Plot every 10th search line to avoid clutter
        plot(Py(:), Px(:), 'm.'); % Magenta dots for search grid
    end
    % ----------------------------------------------------
...
% --- VISUALIZATION 3 (continued): Hold off after search loop ---
hold off;
% -------------------------------------------------------------
